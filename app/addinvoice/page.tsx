...
const NewInvoice = ({ user }: { user: User }) => {
  ...
  useEffect(() => {
    if (form.location_id) {
      fetchProductsByLocation(form.location_id);
    } else {
      setProducts([]);
    }
  }, [form.location_id]);
  ...
  const fetchInitialData = async (): Promise<void> => {
    ...
    try {
      await Promise.all([fetchCustomers(), fetchLocations()]);
    } catch {
  ...
  const fetchProductsByLocation = async (locationId: string): Promise<void> => {
    setIsLoadingProducts(true);
    try {
      const res = await apiGet(`/product-locations/${locationId}`, {}, false);

      console.log("📍 Product location response:", res.data);
      ...
      const mappedProducts: Product[] = Array.isArray(rawProducts) 
        ? rawProducts.map((item: any) => ({
            ...
            name: item.product?.name || item.product_name || 'Unnamed Product',
  ...
  const handleOpenProductSearch = (): void => {
    if (!form.location_id) {
  ...
  const addProduct = (product: Product): void => {
    if (!form.location_id) {
      toast.error("Please select a location before adding products");
      return;
    }
  ...
  const handleLocationChange = (locationId: string): void => {
    ...
    if (errors.location_id) setErrors((prev) => ({ ...prev, location_id: undefined }));
  };
          ...
          <div className="space-y-4">
            ...
            <SectionCard icon={User} title="Customer & Location" subtitle="Select the customer and business location">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <Field label="Location" required error={errors.location_id} hint="Products are loaded based on location">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    disabled={!form.location_id || isLoadingProducts}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    title={!form.location_id ? "Select a location first" : "Add product"}
                  >
                ...
                {!form.location_id && (
                  <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                    ...
                    <p className="text-xs text-amber-700">Please select a location before adding products.</p>
                  </div>
                )}

                {/* Product Search */}
                {showProductSearch && form.location_id && (
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-1">
                        {filteredProducts.slice(0, 10).map((product) => (
                          <button key={product.id} type="button" onClick={() => addProduct(product)}
                            ...
                            <div>
                              <div className="text-sm font-medium text-gray-700">{product.name || 'Unnamed Product'}</div>
                              <div className="text-[11px] text-gray-400 font-mono">{product.sku || 'N/A'}</div>
                              {product.stock_quantity !== undefined && (
                                <div className="text-[11px] text-gray-400">Stock: {product.stock_quantity}</div>
                              )}
                            </div>
