...
const ApproveTransferPage: React.FC<{ user: User }> = ({ user }) => {
    ...
    // Handle approval (for approve/in_transit/suspended actions)
    const handleApprove = async (status: string) => {
        ...
        try {
            ...
            // 🔍 LOG THE APPROVAL PAYLOAD
            console.log('📤 APPROVAL PAYLOAD:', {
                endpoint: `/approve_transfer/${transferId}`,
                method: 'POST',
    ...
    // Handle rejection (separate endpoint)
    const handleReject = async () => {
        ...
        try {
            ...
            // Using the SEPARATE reject endpoint
            const response = await apiPost(
            ...
            if (responseData?.success) {
                ...
                toast.error(responseData?.message || "Failed to reject transfer");
            }
        } catch (error: unknown) {
            const err = error as ExtendedError;
            const message = err.userMessage || err?.response?.data?.message || "Failed to reject transfer";
            ...
            console.error('❌ REJECTION ERROR:', {
                ...
                payload: {
                    notes: rejectionNotes,
                    approved_quantity: approvedQuantity !== transferData?.transfer.stock_quantity ? approvedQuantity : undefined
                },
                timestamp: new Date().toISOString()
            });
