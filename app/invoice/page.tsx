...
interface FilterState {
  search: string;
  status: string;
  sortBy: "invoice_number" | "total_amount" | "created_at" | "due_date" | "invoice_date" | "status" | "payment_status";
  sortOrder: "asc" | "desc";
  dateRange: string;
...
const StatusBadge: React.FC<{ status: Invoice["status"] }> = ({ status }) => {
  const config = {
    ...
    sent: { label: "Sent", icon: Send, cls: "bg-blue-50 text-blue-700 border-blue-200", iconCls: "text-blue-500" },
    paid: { label: "Paid", icon: CheckCircle, cls: "bg-emerald-50 text-emerald-700 border-emerald-200", iconCls: "text-emerald-500" },
    partial: { label: "Partial", icon: Clock, cls: "bg-amber-50 text-amber-700 border-amber-200", iconCls: "text-amber-500" },
  ...
  const { label, icon: Icon, cls, iconCls } = config[status] || config.draft;
  return (
...
const PaymentStatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const config: Record<string, { label: string; icon: React.ElementType; cls: string; iconCls: string }> = {
    unpaid: { label: "Unpaid", icon: XCircle, cls: "bg-rose-50 text-rose-700 border-rose-200", iconCls: "text-rose-500" },
    partial: { label: "Partial", icon: Clock, cls: "bg-amber-50 text-amber-700 border-amber-200", iconCls: "text-amber-500" },
    paid: { label: "Paid", icon: CheckCircle, cls: "bg-emerald-50 text-emerald-700 border-emerald-200", iconCls: "text-emerald-500" },
  };
  const { label, icon: Icon, cls, iconCls } = config[status] || config.unpaid;
  return (
    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold border ${cls}`}>
      <Icon className={`h-3.5 w-3.5 ${iconCls}`} />
      {label}
...
const DropdownPortal: React.FC<{
  ...
  const actions = [
    ...
    { label: "Send to Customer", icon: Send, action: () => {}, show: invoice.status === "sent" },
...
const InvoiceRow: React.FC<{
  ...
  isOpen: boolean;
  onToggleOpen: (id: string | null, position?: { top: number; left: number }) => void;
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            <div className="text-sm font-medium text-stone-700 truncate max-w-[180px]">
              <ShortTextWithTooltip text={getCustomerName()} max={20} />
            </div>
        ...
        <div className="space-y-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-stone-600 font-medium truncate">
              <ShortTextWithTooltip text={getCustomerName()} max={22} />
            </span>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Calendar className="h-4 w-4 text-stone-400" />
              <span className="text-xs text-stone-500">{formatDate(invoice.invoice_date)}</span>
            </div>
            {invoice.due_date && (
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute right-0 top-0 h-full w-80 bg-white shadow-2xl flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <div>
            <label className="text-[11px] font-bold text-stone-400 uppercase tracking-widest mb-2 block">Payment Status</label>
            <select value={filters.paymentStatus} onChange={(e) => onFilterChange("paymentStatus", e.target.value)} className={selectCls}>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white/95 backdrop-blur-sm border-b border-stone-100 px-6 py-4 flex items-center justify-between z-10">
          <div>
            ...
            <p className="text-xs text-stone-400">Created {formatDate(invoice.invoice_date)}</p>
          </div>
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Section icon={User} title="Customer Information">
              <InfoRow label="Name" value={getCustomerName()} />
              {invoice.customer?.email && <InfoRow label="Email" value={invoice.customer.email} />}
              {invoice.customer?.phone && <InfoRow label="Phone" value={invoice.customer.phone} />}
              {invoice.customer?.address && <InfoRow label="Address" value={invoice.customer.address} />}
              {invoice.customer?.city && <InfoRow label="City" value={invoice.customer.city} />}
              {invoice.customer?.state && <InfoRow label="State" value={invoice.customer.state} />}
              {invoice.customer?.country && <InfoRow label="Country" value={invoice.customer.country} />}
              {invoice.customer?.customer_code && <InfoRow label="Customer Code" value={invoice.customer.customer_code} />}
            </Section>

            {/* Business & Location Info */}
            <Section icon={Building2} title="Business & Location">
              {invoice.business?.business_name && <InfoRow label="Business" value={invoice.business.business_name} />}
              {invoice.location?.location_name && <InfoRow label="Location" value={invoice.location.location_name} />}
              {invoice.location?.address && <InfoRow label="Address" value={invoice.location.address} />}
              {invoice.location?.phone && <InfoRow label="Phone" value={invoice.location.phone} />}
            </Section>
            ...
            <Section icon={Banknote} title="Payment Details">
              ...
              <InfoRow label="Amount Paid" value={formatCurrency(invoice.amount_paid)} />
              <InfoRow label="Balance Due" value={<span className={invoice.balance_due > 0 ? "text-rose-600" : "text-emerald-600"}>{formatCurrency(invoice.balance_due)}</span>} />
              <InfoRow label="Payment Status" value={<PaymentStatusBadge status={invoice.payment_status} />} />
              {invoice.payment_method && <InfoRow label="Payment Method" value={invoice.payment_method} />}
              {invoice.transaction_reference && <InfoRow label="Transaction Ref" value={invoice.transaction_reference} />}
            </Section>
            ...
            <Section icon={Calendar} title="Important Dates">
              ...
              {invoice.frequency && (
                <InfoRow label="Recurring" value={`${invoice.frequency} (every ${invoice.interval})`} />
              )}
            </Section>
          </div>
          ...
          {invoice.staff && (
            <div className="mt-6">
              <h3 className="text-sm font-bold text-stone-800 mb-2">Staff</h3>
              <div className="bg-stone-50 rounded-xl p-4 border border-stone-100">
                <p className="text-sm text-stone-700 font-medium">
                  {invoice.staff.first_name} {invoice.staff.last_name}
                </p>
                <p className="text-xs text-stone-500">{invoice.staff.email}</p>
              </div>
            </div>
          )}
...
const ManageInvoices = ({ user }: { user: User }) => {
  ...
  }, [debouncedSearch, filters.status, filters.sortBy, filters.sortOrder, filters.dateRange, filters.paymentStatus]);
  ...
  const fetchInvoices = useCallback(async (page = 1, perPage = 25) => {
    ...
    try {
      ...
      if (res.data?.pagination) {
        const invoicesData = Array.isArray(res.data.data) ? res.data.data : [];
  ...
  const getEditUrl = (inv: Invoice) => `/editinvoice/${inv.id}`;

  // Calculate statistics
  const stats = useMemo(() => {
    const totalAmount = invoices.reduce((sum, inv) => sum + (inv.status === "paid" ? inv.total_amount : 0), 0);
    const pendingAmount = invoices.reduce((sum, inv) => sum + (inv.payment_status !== "paid" ? inv.balance_due : 0), 0);
    ...
    <div className="min-h-screen bg-stone-100">
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="flex items-center gap-2 mb-5 flex-wrap">
          {["All", "Draft", "Sent", "Paid", "Partial", "Overdue", "Cancelled"].map((label) => {
